...
export default function BasicInfoStep() {
  ...
  const getInputClassName = (fieldName: keyof BasicInfoData) => {
    ...
    switch (status) {
      ...
      default:
        return `${baseClasses} border-gray-600/30 focus:ring-[#FF2800]/30 focus:border-[#FF2800]`;
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        <label className="flex items-center text-sm font-medium text-gray-200 mb-3">
          <div className="w-6 h-6 bg-[#FF2800]/20 rounded-lg flex items-center justify-center mr-2">
            <Car className="h-4 w-4 text-[#FF2800]" />
          </div>
          Car Make *
        </label>
        ...
        <div className="grid grid-cols-5 gap-3">
          {[2, 4, 5, 7, 9].map((seatCount) => (
            <button
              ...
              className={`py-3 px-2 rounded-xl border-2 transition-all duration-200 text-sm font-semibold relative overflow-hidden ${
                watchedValues.seats === seatCount
                  ? 'bg-[#FF2800] border-[#FF2800] text-white shadow-lg transform scale-105'
            ...
            >
              ...
              {watchedValues.seats === seatCount && (
                <div className="absolute inset-0 bg-gradient-to-r from-[#FF2800] to-[#FF2800]/80" />
              )}
            </button>
          ))}
        </div>
      ...
      {watchedValues.make && watchedValues.model && watchedValues.year && (
        <GlassCard gradient="accent" className="border-2">
          <div className="flex items-start space-x-3">
            <div className="w-12 h-12 bg-[#FF2800]/20 rounded-xl flex items-center justify-center">
              <Car className="h-6 w-6 text-[#FF2800]" />
            </div>
            <div className="flex-1">
              <h3 className="text-sm font-medium text-[#FF2800] mb-1">Car Preview</h3>
